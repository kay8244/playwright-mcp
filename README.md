...
의존성은 `@playwright/mcp` 하나뿐 → 어느 PC에 풀어도 그대로 동작한다.
...
- **Claude Code** — 이미 사용 중이면 OK

---

## A. 온라인 설치 (npm/CDN 접속 가능할 때) — 한 줄이면 끝
```bash
git clone https://github.com/kay8244/playwright-mcp.git
cd playwright-mcp
npm run setup     # npm install + 크로미움 + 자체검증(verify)까지 한 방
...
```powershell
npm install
$env:PLAYWRIGHT_BROWSERS_PATH="0"     # ← PowerShell 방식. set 아님!
...
```bat
npm install
set PLAYWRIGHT_BROWSERS_PATH=0 && npx playwright install chromium
node verify.mjs
claude
```
**mac/Linux**:
...
이 폴더를 통째로 zip 하면 별도 패커 없이 자체완결 번들이 된다.
...
cd playwright-mcp
npm run setup
#  → node_modules 안에 @playwright/mcp + 크로미움까지 설치됨
#  이 playwright-mcp 폴더(node_modules 포함) 전체를 zip → USB 또는 GitHub Release 로 반입
```
```bat
:: [폐쇄망 Windows PC]  zip 을 폴더째 해제 후
run-closed-win.bat      :: 네트워크 0 자체검증
claude                  :: MCP approve 후 사용
```
> ⚠ 준비 PC와 사내 PC의 **OS/CPU가 같아야** 한다(Windows x64 등). mac에서 받은 크로미움은 Windows에서 못 쓴다.
> 사내에 Node 가 없으면 https://nodejs.org 의 `.msi` 도 함께 반입해 먼저 설치한다.
...
- `run-closed-win.bat` — 폐쇄망 Windows 원클릭 실행(node 체크→검증→안내)
...
| 증상 | 원인 / 해결 |
|---|---|
| `node: command not found` | Node 18+ 미설치. https://nodejs.org 에서 설치 후 재시도 |
